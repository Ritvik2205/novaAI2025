"""
Adapted crawler utilities derived from the legacy RAG implementation.

This version keeps the SiteCrawler API while delegating to the modern settings module.
"""
...
class SiteCrawler:
            ...
            async def process(url: str, depth: int) -> None:
                ...
                try:
                    async with semaphore:
                        response = await client.get(url, headers={"User-Agent": "AgenticCRMbot/1.0"})
    ...
    async def _load_sitemap_urls(self) -> List[str]:
        ...
        async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
            for candidate in candidate_paths:
                ...
                try:
                    response = await client.get(sitemap_url, headers={"User-Agent": "AgenticCRMbot/1.0"})
    ...
    async def _load_robot_rules(self) -> None:
        ...
        async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
            try:
                response = await client.get(robots_url, headers={"User-Agent": "AgenticCRMbot/1.0"})
            except httpx.HTTPError:
                return
        if response.status_code == httpx.codes.OK:
